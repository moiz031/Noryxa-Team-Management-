...
const patchCompanyDocSchema = z.object({
  ...
  category: z.enum(["company", "general"]).optional(),
});
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = patchCompanyDocSchema.safeParse(body);
    if (!parsed.success) {
    ...
    await createSupabaseAdminClient()
      ...
      .insert({
        ...
        action_type: oldStoragePathToRemove ? "company_document.replaced" : "company_document.renamed",
