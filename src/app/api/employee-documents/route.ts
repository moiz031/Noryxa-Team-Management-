...
const employeeDocSchema = z.object({
  ...
  category: z.enum(['employee', 'general']).default('employee'),
});
...
export async function GET(request: Request) {
  try {
    ...
    let query = auth.supabase.from("employee_documents").select("*");
...
async function postHandler(request: Request) {
  try {
    ...
    const parsed = employeeDocSchema.safeParse(body);
    if (!parsed.success) {
    ...
    // Use user's RLS to insert metadata
    ...
    await createSupabaseAdminClient()
      ...
      .insert({
        ...
        action_type: "employee_document.uploaded",
        entity_type: "employee_document",
        entity_id: document.id,
        metadata: { category: document.category, file_name: document.file_name, employee_id: employee.id }
      });
