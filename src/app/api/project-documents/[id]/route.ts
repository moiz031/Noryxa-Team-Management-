...
const patchProjectDocSchema = z.object({
  ...
  category: z.enum(["project", "general"]).optional(),
});
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = patchProjectDocSchema.safeParse(body);
    if (!parsed.success) {
    ...
    await createSupabaseAdminClient()
      ...
      .insert({
        ...
        action_type: oldStoragePathToRemove ? "project_document.replaced" : "project_document.renamed",
